...
MACHINES = [
    ("mini-ekskavator", "Mini Ekskavatör", "Dar alan ve hafif hafriyat işleri için kompakt ekskavatörler.",
     "Mini ekskavatörler, dar sokak aralarında, bahçe düzenlemesinde, bina içi yıkım ve küçük çaplı hafriyat işlerinde manevra kabiliyeti sağlar. GND İş Makineleri olarak sıfır ve 2. el mini ekskavatör tedarikinde geniş üretici ağımızla size en uygun teklifi sunuyoruz.", "excavator", None),
    ("orta-ekskavator", "Orta Seviye Ekskavatör", "Genel hafriyat ve inşaat işleri için orta sınıf ekskavatörler.",
     "Orta seviye ekskavatörler, konut ve ticari inşaat projelerinde, altyapı çalışmalarında ve genel hafriyat işlerinde en çok tercih edilen makine sınıfıdır. Sıfır ve 2. el seçeneklerle, ihtiyacınıza uygun tonaj ve modeli sizin için buluyoruz.", "excavator", None),
    ("agir-tonaj-ekskavator", "Ağır Tonaj Ekskavatör", "Maden ve büyük ölçekli hafriyat için ağır tonajlı ekskavatörler.",
     "Ağır tonajlı ekskavatörler; maden ocakları, büyük ölçekli hafriyat sahaları ve taş ocağı operasyonları için yüksek kazı kapasitesi sunar. Global tedarik ağımız sayesinde ağır tonaj sınıfında sıfır ve 2. el makineler için rekabetçi teklifler hazırlıyoruz.", "excavator", None),
    ("loader", "Loaderlar", "Lastikli yükleyiciler, her ölçekte yükleme işi için.",
     "Lastikli yükleyiciler (loader), malzeme yükleme, taşıma ve istifleme operasyonlarında saha verimliliğini artırır. GND İş Makineleri, farklı kapasite ve marka seçenekleriyle sıfır ve 2. el loader tedariki sağlar.", "loader", None),
    ("bekoloader", "Bekoloaderler", "Kazı ve yükleme işini tek makinede birleştiren bekoloaderler.",
     "Bekoloaderler, kazı ve yükleme fonksiyonlarını tek bir makinede birleştirerek küçük ve orta ölçekli şantiyelerde çok yönlü kullanım sağlar. İhtiyacınıza uygun bekoloader modelini üretici ağımızdan bulup teklif hazırlıyoruz.", "backhoe-loader", None),
    ("manlift", "Manliftler", "Yüksekte çalışma platformları, bakım ve montaj işleri için.",
     "Manliftler (yüksekte çalışma platformları), bakım, montaj, elektrik ve boya işlerinde güvenli ve verimli erişim sağlar. GND İş Makineleri olarak makaslı ve eklemli manlift seçenekleri için teklif sunuyoruz.", "manlift", None),
    ("telehandler", "Telehandlerlar", "Teleskopik kollu yükleyiciler, yüksek ve uzak noktalara taşıma için.",
     "Telehandlerlar, teleskopik kolları sayesinde yüksek ve uzak noktalara malzeme taşımada esneklik sağlar; inşaat, tarım ve depo operasyonlarında tercih edilir. Sıfır ve 2. el telehandler tedariki için bize ulaşın.", "telehandler", None),
    ("silindir", "Silindirler", "Zemin ve asfalt sıkıştırma işleri için silindirler.",
     "Silindirler, yol yapımı, zemin ve asfalt sıkıştırma işlerinde vazgeçilmez makinelerdir. GND İş Makineleri, tek ve çift tamburlu silindir seçenekleriyle projelerinize uygun tedarik sağlar.", "roller", None),
    ("finiser", "Finişerler", "Asfalt serimi için finişer makineleri.",
     "Finişerler, asfalt serim kalitesini ve hızını doğrudan etkileyen kritik yol yapım makineleridir. Üretici ağımız üzerinden sıfır ve 2. el finişer tedariki konusunda size destek oluyoruz.", "finisher", None),
    ...
    ("dozer", "Dozerler", "Ağır zemin çalışmaları için güçlü dozer seçenekleri.",
     "Dozerler, ağır zemin itme, tesviye ve hafriyat hazırlığı işlerinde yüksek güç ve verim sağlar. Sıfır ve 2. el dozer seçenekleri için GND İş Makineleri'nden teklif alabilirsiniz.", "dozer", None),
    ("skid-steer-loader", "Skid Steer Loaderlar", "Dar alanlarda manevra kabiliyeti yüksek kompakt yükleyiciler.",
     "Skid steer loaderlar, dar ve engebeli alanlarda yüksek manevra kabiliyeti sunan kompakt yükleyicilerdir; peyzaj, iç mekan ve küçük şantiye işlerinde tercih edilir. Uygun modeli sizin için buluyoruz.", "skid-steer", None),
]
